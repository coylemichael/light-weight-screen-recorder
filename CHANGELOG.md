...
- **Two new auto-clip kill templates: `finisher.png` and `runner_elim.png`** — Match alongside `runner_down.png` so Marathon finisher and elimination banners trigger clip saves in addition to the standard "RUNNER DOWN" banner. `MAX_TEMPLATES` in `src/kill_feed_sampler.c` raised from 2 to 4; two extra `LoadTemplatePNG` calls in `KillFeedSampler_Init`. Each template is loaded independently — a missing PNG logs a warning and skips that template rather than disabling the sampler. Assets present in both `static\` and `bin\static\`.
...
### Fixed
- **Replay buffer silently went stale after ~25.6 hours of continuous capture** — `BufferThreadProc` in `src/replay_buffer.c` computed the wall-clock PTS for each frame as `(currentTime - captureStartTime) * 10000000LL / perfFreq.QuadPart`. On a 10 MHz QPC (Windows default) the `delta * 1e7` intermediate overflows `LONGLONG` at ~25.62 h, after which `currentSlot` becomes garbage and the CFR slot-skip check `currentSlot <= lastSubmittedSlot` is permanently true. The buffer thread kept iterating (heartbeats fine, audio fine) but stopped submitting frames, so the replay buffer slowly went stale on disk — overnight sessions had auto-clip silently dead the following day, and manual saves returned hours-old video. New `Util_QpcDeltaToHns` helper in `src/util.c` / `src/util.h` uses split-divide arithmetic that is safe for ~29 000 years on a 10 MHz QPC; the broken site is rewritten to use it. Also added a staleness self-check at the top of the buffer loop that emits a `WARNING` if `attemptCount` advances for ≥ 10 s with `frameCount` frozen, so any future "loop alive but producing nothing" failure surfaces in the log within seconds instead of going unnoticed for an entire session.
- **Audio leads video by ~0.5–1 s in saved replay clips** — `HandleSaveRequest` in `src/replay_buffer.c` was rebasing each stream to its own first sample's PTS independently: video skipped to the first keyframe (up to one GOP = `fps/2` frames ≈ 0.5 s lost from the front), audio rebased to its oldest kept sample, and each per-source track rebased to *its* oldest sample. After per-stream rebase to PTS = 0, every track started at a different wall-clock instant. The capture-side QPC anchor was also being taken twice — once inside `AudioCapture_Start` and once in `BufferThreadProc` after the audio init — adding tens of ms of additional drift. Fix: capture the QPC anchor once before audio init and pass it to `AudioCapture_StartAt` (replacing `AudioCapture_Start`); `CopyAudioSamplesForMuxing` and `CopyPerSourceSamplesForMuxing` preserve absolute (since-`t0`) PTS instead of rebasing; new `AlignAudioToVideoWindow` helper rebases every audio track against `videoOriginTs` (the absolute PTS of the first kept video frame, now wired through `FrameBuffer_GetFramesForMuxing`'s `outOriginTimestamp` out-param that was added in 1.4.1 but unused). Samples fully before `t = videoOriginTs` are dropped, samples that straddle the front edge clamp to `t = 0`, samples fully after the video window are dropped. All five tracks (video + mixed audio + up to three per-source) now share a left edge.
- **Wrong-monitor recordings after KVM swap / display topology change** — `Capture_ReinitDuplication` (`src/capture.c`) used to re-bind to `state->monitorIndex` (a DXGI output index) on every `DXGI_ERROR_ACCESS_LOST`, with no check that that index still referred to the same physical monitor. After a KVM swap (or any topology event that re-enumerated DXGI outputs), the recorder could silently start capturing a different output while the encoder kept producing the originally-configured frame size. Now identifies the capture target by **PnP device ID** (EDID-derived hardware identity) resolved via `EnumDisplayDevices` at first bind; reinit and the `Capture_SetRegion` output-switch path both refuse to bind to any output whose PnP ID does not match the locked target, leaving `accessLost = TRUE` so the buffer thread keeps polling until the real monitor reappears. Falls back to legacy index-based behaviour for displays with no PnP ID (RDP / virtual displays). New `targetMonitorId[256]` field on `CaptureState` + new static helper `FindTargetOutput` in `src/capture.c`.
...
- **Audio/video origin mismatch in replay buffer** — `AudioCapture_StartAt(ctx, t0)` shares the same QPC reference between audio and video so PTS values align. `FrameBuffer_GetFramesForMuxing` now returns `originTimestamp` for audio rebase.
...
- `Config_GetPath()` declaration from config.h - made static in config.c (only used internally)
